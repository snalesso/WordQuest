...
import { CategoryHeader, MatchSettings, MatchSnapshot } from '../models/game.DTOs';
...
export class GameService extends NcbApiService {
  ...
  public getCategoryHeadersAsync(/*languageId: Language["id"],*/ alphabetId: Alphabet["id"]) {
    ...
    if (!!alphabetId)
      queryParams = queryParams.append("alphabetId", alphabetId.toString());

    return this.http.get<readonly CategoryHeader[]>(this.getEndpoint("GetCategoryHeaders"), { params: queryParams })
      .pipe(
        forceDelayInDev(),
        //   map(categs => {
        //     return categs;
  ...
  //       // tap(x => logEvent(this, 'alphabet options', x)),
