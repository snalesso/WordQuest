import { BehaviorSubject, combineLatest, defer, from, Observable, ObservableInput, of, Subject } from 'rxjs';
...
import { logEvent } from '../../utils/dev.utils';
...
export class ObservableValueSource<
    ...
    public readonly isLoading$ = this._isLoading$$
        .pipe(
            ...
            tap(x => this.conditionallyLog("Is loading", x)),
            shareReplay({ bufferSize: 1, refCount: true }));

    // @Output()
    public readonly canReload$: Observable<boolean> = this.isLoading$
        .pipe(
            map(() => this.canReload),
            distinctUntilChanged(),
            tap(x => this.conditionallyLog("Can reload", x)),
            shareReplay({ bufferSize: 1, refCount: true }));
    public get canReload() { return this._isLoading$$.value === false; }

    private readonly _reloadTrigger$$ = new Subject<void>();
    // @Output()
    public readonly reloadSignal$ = this._reloadTrigger$$
        .pipe(
    ...
    // @Output()
    public readonly hasError$ = this._hasError$$
        .pipe(
            distinctUntilChanged(),
            tap(x => this.conditionallyLog("Has error", x)),
            shareReplay({ bufferSize: 1, refCount: true }));

    // @Output()
    public readonly canReset$: Observable<boolean> = combineLatest([this.isLoading$, this.hasError$])
        .pipe(
            map(() => this.canReset),
            distinctUntilChanged(),
            tap(x => this.conditionallyLog("Can reset", x)),
            shareReplay({ bufferSize: 1, refCount: true }));
    public get canReset() { return this.isLoading === false && this.hasError === false; }

    private readonly _resetTrigger$$ = new Subject<void>();
    // @Output()
    public readonly resetSignal$ = this._resetTrigger$$
        .pipe(
            takeWhile(() => this.canReload),
    ...
    protected readonly _value$ = defer(
        ...
        .pipe(
            ...
            tap(x => this.conditionallyLog("Data emitted", x, true)),
            shareReplay({ bufferSize: 1, refCount: true }));
    ...
    public readonly hasValue$ = this.value$
        .pipe(
            ...
            tap(x => this.conditionallyLog("Has value", x)),
            shareReplay({ bufferSize: 1, refCount: true }));
