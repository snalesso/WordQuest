...
internal class EfCoreLanguageOptionsView : ILanguageOptionsView
{
    private readonly DbSet<EfCoreLanguage> _languages;

    public EfCoreLanguageOptionsView(DbSet<EfCoreLanguage> languages)
    {
        this._languages = languages;
    }

    public async Task<ILanguageOption> GetAsync(short id, CancellationToken cancellationToken = default)
    {
        var lang = await this._languages
            .FindAsync(id, cancellationToken)
            .ConfigureAwait(false);
        if (lang is null)
            throw new Exception("Not found.");
        var result = new LanguageOption(lang.Id, lang.NativeName);
        return result;
    }

    public async Task<IReadOnlyList<ILanguageOption>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var langs = await this._languages
            .Select(lang => new LanguageOption(lang.Id, lang.NativeName))
