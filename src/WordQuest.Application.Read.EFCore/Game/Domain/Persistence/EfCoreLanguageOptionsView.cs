...
public class EfCoreLanguageOptionsView : ILanguageOptionsView
{
    private readonly DbSet<Language> _languages;

    public EfCoreLanguageOptionsView(DbSet<Language> languages)
    {
        this._languages = languages;
    }

    public async Task<Domain.LanguageOption?> GetAsync(short id, CancellationToken cancellationToken = default)
    {
        var lang = await this._languages.FindAsync(id, cancellationToken);

        Domain.LanguageOption? result = lang is null ? null : new Domain.LanguageOption(lang.Id, lang.NativeName);
        return result;
    }

    public async Task<IReadOnlyList<Domain.LanguageOption>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var langs = await this._languages.ToArrayAsync(cancellationToken);
        return langs.Select(lang => new Domain.LanguageOption(lang.Id, lang.NativeName)).ToArray();
    }
